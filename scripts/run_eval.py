...
def run_single_query(
    ...
    try:
        ...
        # is not supported on Windows, so threading + queue is used instead.
        line_queue: queue.Queue = queue.Queue()

        def _read_stdout(stdout, q):
            try:
        ...
        reader = threading.Thread(
            target=_read_stdout, args=(process.stdout, line_queue), daemon=True
        )
        reader.start()
        ...
        try:
            while time.time() - start_time < timeout:
                ...
                if raw is None:  # sentinel — stdout closed
                    break
                ...
                if event.get("type") == "stream_event":
                    ...
                    if se_type == "content_block_start":
                        ...
                        if delta.get("type") == "input_json_delta":
                            ...
                            # Match the real installed skill name OR the temp
                            # command file name (for skills not yet installed).
                            if skill_name in accumulated_json or clean_name in accumulated_json:
                                return True

                    elif se_type in ("content_block_stop", "message_stop"):
                        if pending_tool_name:
                            return skill_name in accumulated_json or clean_name in accumulated_json
                ...
                # Fallback: full assistant message
                    ...
                    for content_item in message.get("content", []):
                        ...
                        if tool_name == "Skill" and (
                            skill_name in skill_val or clean_name in skill_val
                        ):
                            triggered = True
                        elif tool_name == "Read" and (
                            skill_name in file_val or clean_name in file_val
                        ):
                            triggered = True
                        return triggered
